...
class AkClient()(implicit system: ActorSystem) extends StrictLogging {
  ...
  def getPossibleCharacters(session: SessionInfo): Future[Unit] = {
    // todo check for useless params
    lazy val url = s"$baseUrl/ws/list?session=${session.id}&signature=${session.signature}&step=${session.step}&size=1&max_pic_width=246&max_pic_height=294&pref_photos=OK-FR&mode_question=0"
